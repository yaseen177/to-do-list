...
import { signInWithPopup, signInWithEmailAndPassword, createUserWithEmailAndPassword, sendPasswordResetEmail, updateProfile, confirmPasswordReset, GoogleAuthProvider } from "firebase/auth";
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (params.get('mode') === 'resetPassword') { setMode('reset'); setOobCode(params.get('oobCode') || ''); }
  }, []);
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        ...
        const top = priorityCodes.map(code => formatted.find(c => c.code === code)).filter(Boolean) as FormattedCountry[];
        const others = formatted.filter(c => !priorityCodes.includes(c.code));
  ...
  const handleGoogleLogin = async () => {
    ...
    try { 
      ...
      if (token) {
        // Save Calendar Token for Dashboard
        localStorage.setItem(`google_token_${result.user.uid}`, token);
      }
      ...
      <div className="glass-panel p-8 w-full max-w-md relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <h1 className="text-2xl font-bold mb-2">
            {mode === 'signin' ? 'Welcome Back' : mode === 'signup' ? 'Create Account' : mode === 'forgot' ? 'Reset Password' : 'New Password'}
          </h1>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4 text-left">
            ...
            {mode === 'signup' && (
              <>
                <div className="relative">
                  ...
                  <input type="text" placeholder="Full Name" required value={fullName} onChange={e => setFullName(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-3 pl-10 pr-4 text-sm focus:border-indigo-500 outline-none" />
                </div>
                <div className="flex gap-2">
                  <div className="relative w-[110px]">
                    {loadingCountries ? (
                      <div className="w-full h-full bg-slate-900/50 border border-slate-700/50 rounded-xl flex items-center justify-center"><Loader2 size={16} className="animate-spin text-slate-500"/></div>
                    ) : (
                      <>
                        <select value={countryCode} onChange={e => setCountryCode(e.target.value)} className="w-full h-full bg-slate-900/50 border border-slate-700/50 rounded-xl pl-8 pr-2 py-3 text-sm appearance-none outline-none cursor-pointer">
                          {countries.map(c => <option key={`${c.code}-${c.dial_code}`} value={c.dial_code}>{c.dial_code}</option>)}
                        </select>
                        <div className="absolute left-2 top-1/2 -translate-y-1/2 pointer-events-none flex items-center">
                           <img src={countries.find(c => c.dial_code === countryCode)?.flag} alt="flag" className="w-5 h-3.5 object-cover rounded-sm"/>
                        </div>
                      </>
                    )}
                  </div>
                  <div className="relative flex-1">
                    ...
                    <input type="tel" placeholder="Mobile Number" required value={phone} onChange={e => setPhone(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-3 pl-10 pr-4 text-sm focus:border-indigo-500 outline-none" />
                  </div>
                </div>
              </>
            )}
            ...
            {mode !== 'reset' && (
              <div className="relative">
                ...
                <input type="email" placeholder="Email Address" required value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-3 pl-10 pr-4 text-sm focus:border-indigo-500 outline-none" />
              </div>
            )}
            ...
            {mode !== 'forgot' && (
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-3 pl-10 pr-4 text-sm focus:border-indigo-500 outline-none" 
                />
              </div>
            )}
            ...
            {(mode === 'signup' || mode === 'reset') && (
              <>
                ...
                <div className="relative">
                  <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
                  <input type="password" placeholder="Confirm Password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-3 pl-10 pr-4 text-sm focus:border-indigo-500 outline-none" />
                </div>
              </>
            )}
          ...
          {(mode === 'signin' || mode === 'signup') && (
            <>
              <div className="my-6 flex items-center gap-4 opacity-50"><div className="h-[1px] bg-slate-600 flex-1"></div><span className="text-xs text-slate-400">OR</span><div className="h-[1px] bg-slate-600 flex-1"></div></div>
          ...
          <div className="mt-6 text-sm text-slate-400 space-y-2">
            {mode === 'signin' && <><button onClick={() => setMode('forgot')} className="hover:text-white transition">Forgot password?</button><div>Don't have an account? <button onClick={() => setMode('signup')} className="text-indigo-400 hover:text-indigo-300 font-medium">Sign up</button></div></>}
            {mode === 'signup' && <div>Already have an account? <button onClick={() => setMode('signin')} className="text-indigo-400 hover:text-indigo-300 font-medium">Sign in</button></div>}
            {(mode === 'forgot' || mode === 'reset') && <button onClick={() => setMode('signin')} className="flex items-center justify-center gap-2 hover:text-white transition mx-auto"><ArrowLeft size={14} /> Back to Sign In</button>}
          </div>
        </div>
      </div>
    </div>
